...
int graph_DFS(Graph graph, Vertex vertex, List visited, List recStack) {
    static int depth = 0;
    depth++;
    printf("DFS depth: %d, visiting vertex %d\n", depth, vertex->iban);

    // Check if vertex has edges
    if (vertex->edges == NULL) {
        printf("Warning: vertex->edges is NULL for vertex %d\n", vertex->iban);
        depth--;
        return 0;
    }

    // Traverse edges
    for (ListNode node = list_first(vertex->edges); node != NULL; node = list_next(vertex->edges, node)) {
        Edge edge = (Edge)node->value;
        if (edge == NULL) {
            printf("Warning: Edge is NULL.\n");
            continue;
        }
        printf("Processing edge with destination IBAN %d\n", edge->destination);

        // Find the actual destination vertex in the graph
        Vertex temp = list_find(graph->vertices, create_int(edge->destination), compare_vertices);
        if (temp == NULL) {
            printf("Error: Destination vertex with IBAN %d not found in graph vertices.\n", edge->destination);
            continue;
        }
        printf("Found destination vertex with IBAN %d, Address: %p\n", temp->iban, (void *)temp);

        // Check if this vertex is in visited or recStack
        if (list_find(visited, temp, compare_vertices) == NULL) {
            // Recursive DFS call if the vertex hasn't been visited
            if (graph_DFS(graph, temp, visited, recStack)) {
                depth--;
                return 1;  // Cycle detected, propagate upwards
            }
    ...
    if (recNode != NULL) {
        ...
        printf("Warning: Vertex %d not found in recStack for deletion.\n", vertex->iban);
    }
